...
- 类 Apple 液态玻璃风格：使用真正透明的无边框 WebView、圆角裁切、动态边缘高光与指针折射，并适配深浅色、高对比度和减少动态效果设置。
...
- 后台每 10 秒检测 `claude.exe`：Claude Code 启动时显示小组件，退出时自动隐藏。
...
Key 不会发送到其他网络地址，也不会写入应用日志。第三方响应先在 Rust 后端经过 256 KiB 上限与类型化解析，WebView 只接收两条归一化指标，不接收供应商原始 JSON。
